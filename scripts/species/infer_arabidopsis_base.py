...
from mocps.config import BASE_MODEL_DIR, DATA_DIR, RESULTS_DIR
...
DEFAULT_OUTPUT_CSV = RESULTS_DIR / "arabidopsis_thaliana_with_base_check.csv"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Arabidopsis base-model inference on eval CSV")
...
def main():
    ...
    df["my_base_infer_dna"] = pred_dna
    df["my_base_valid_triplet"] = valid_triplet
    df["my_base_has_stop"] = has_stop
    df["my_base_translated_match"] = translated_match
